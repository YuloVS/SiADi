        ...
        private void filtrar()
        {
            using (var db = new SiADiDB())
            {
                ...
                { 
                    ...
                    var consulta = from p in db.Personas
                        join c in db.Cargos on p.Cargo.Id equals c.Id
                        where (p.Cuil == textNum || p.Nombre.Contains(textBoxFiltro.Text) || p.Apellido.Contains(textBoxFiltro.Text) || c.Nombre.Contains(textBoxFiltro.Text) || p.Telefono == textNum) && p.Cargo.AreaId == usuario.Cargo.AreaId
                        select new {Id = p.Id, Cuil = p.Cuil, Nombre = p.Nombre, Apellido = p.Apellido, cNombre = c.Nombre, Telefono = p.Telefono};
                    var list = consulta.ToArray();
                    if (list.Length > 0)
                    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.CurrentRow != null)
            {
                ...
                using (var db = new SiADiDB())
                {
                    ...
                    Form modificarUsuario = new imagen(this.usuario, admin, usuario);
